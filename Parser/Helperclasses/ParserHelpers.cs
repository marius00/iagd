...
namespace IAGrim.Parser.Helperclasses {
        ...
        public static string GetBitmap(IEnumerable<T> Tags) {
            ...
            if (Tags != null) {
                if (Tags.Any(m => "bitmap".Equals(m.Stat)))
                    result = Tags.FirstOrDefault(m => "bitmap".Equals(m.Stat)).TextValue;

                else if (Tags.Any(m => "relicBitmap".Equals(m.Stat)))
                    result = Tags.FirstOrDefault(m => "relicBitmap".Equals(m.Stat)).TextValue;

                else if (Tags.Any(m => "shardBitmap".Equals(m.Stat)))
                    result = Tags.FirstOrDefault(m => "shardBitmap".Equals(m.Stat)).TextValue;

                else if (Tags.Any(m => m.Stat.Contains("itmap")))
                    result = Tags.FirstOrDefault(m => m.Stat.Contains("itmap")).TextValue;
            }
    ...
    public class ParserHelpers<T> where T : IItemStat {
        public static string GetBitmap(IEnumerable<T> Tags) {
            ...
            return $"{System.IO.Path.GetFileName(result.Replace(".dbr", ".tex"))}.png";
